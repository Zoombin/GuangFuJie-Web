...
if(1||($ua == '' || preg_match($uachar, $ua)) && !strpos(strtolower($_SERVER['REQUEST_URI']),'wap'))
{
	...
	class alipay
	{
		...
		function get_code($order, $payment)
		{
			...
			$button = '<div style="text-align:center"><input class="btn btn-warning btn-lg" type="button" onclick="window.open(\'https://mapi.alipay.com/gateway.do?'.$param. '&sign='.md5($sign).'&sign_type=MD5\')" value="' .$GLOBALS['_LANG']['pay_button']. '" /></div>';
