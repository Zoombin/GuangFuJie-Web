...
/**
 * ecs_verifycode.status
 * 1: 注册时的验证码
 * 2: 
 * 3: 重置密码时发给用户的短信, 包含用户的新密码
 * 4/5: 绑定用户名和手机号 (现在没有使用)
 * 6: 找回密码页面的验证码
 */
...
// forget password
elseif ($_REQUEST['step'] == 'forgetPwd')
{
    $sql = "select user_id from " . $ecs->table('users') . " where mobile_phone='" . $mobile . "'";
    $user_id = $db->getOne($sql);
    if (empty($user_id)) {
        die($json->encode(array('error' => 4, 'message' => "手机号码不存在\n无法通过该号码找回密码")));
    }
    /* 获取验证码请求是否获取过 */
    $sql = "SELECT COUNT(id) FROM " . $ecs->table('verifycode') ." WHERE status=6 AND getip='" . real_ip() . "' AND dateline>'" . gmtime() ."'-".$_CFG['ecsdxt_smsgap'];
    ...
    $tpl = "您的验证码是：{$verifycode}，请不要把验证码泄露给其他人，如非本人操作，可不用理会！";
    // $content = $smarty->fetch('str:' . $_CFG['ecsdxt_mobile_bind_value']);
    $content = $smarty->fetch('str:' . $tpl);
